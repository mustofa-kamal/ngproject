...
app.post('/convert', async (req, res) => {
  ...
  try {
    ...
    if (text.trim() === '') {
      ...
      return res.status(400).send('No text found in the file');
    }
    ...
    for (const chunk of textChunks) {
      const request = {
        input: { text: chunk },
        voice: { languageCode: 'en-US', name: 'en-US-Standard-D', ssmlGender: 'MALE' }, // Newscast style voice
        audioConfig: {
          audioEncoding: 'MP3',
          speakingRate: 0.88,
          pitch: 0.0,
    ...
    res.status(500).send(`Error converting text to audio: ${error.message}`);
  }
});
